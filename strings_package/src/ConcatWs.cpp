...
class ConcatWs : public TransformFunction
{
                                  ...
                                  PartitionWriter &output_writer)
    {
        if (input_reader.getNumCols() < 2)
            vt_report_error(0, "Function only accepts more then 1 arguments, but %zu provided", input_reader.getNumCols());

        ostringstream oss;
        bool first = true;
        bool exceeded = false;
        std::string separator;
        const VString &separatorElem = input_reader.getStringRef(0);
        separator = separatorElem.str();

        for (int i=1; i<input_reader.getNumCols()) {
...
class ConcatWsFactory : public TransformFunctionFactory
{
    ...
    { return vt_createFuncObj(srvInterface.allocator, ConcatWs); }

};

RegisterFactory(ConcatWsFactory);
